"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.validateAddressMiddleware = void 0;
const responseBase_1 = __importDefault(require("../dtos/responseBase"));
const varHelper_1 = __importDefault(require("../helpers/varHelper"));
const validateAddressMiddleware = (walletService) => {
    return (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
        const requestBody = req.body;
        if (!requestBody.walletAddress || !requestBody.networkType) {
            var response = new responseBase_1.default(varHelper_1.default.HttpStatusCodes.BadRequest, varHelper_1.default.ResponseStatus.ERROR, "Missing wallet or network type");
            return res.status(varHelper_1.default.HttpStatusCodes.BadRequest).json(response);
            // return res.status(400).send("Missing wallet or network");
        }
        if (!(yield walletService.IsValidWallet(requestBody))) {
            var response = new responseBase_1.default(varHelper_1.default.HttpStatusCodes.BadRequest, varHelper_1.default.ResponseStatus.ERROR, "Invalid wallet address");
            return res.status(varHelper_1.default.HttpStatusCodes.BadRequest).json(response);
        }
        next();
    });
};
exports.validateAddressMiddleware = validateAddressMiddleware;
//# sourceMappingURL=validateAddressMiddleware.js.map